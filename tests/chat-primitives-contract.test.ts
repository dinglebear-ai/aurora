...
test("new chat surfaces preserve Aurora conversation ergonomics", () => {
  ...
  assert.ok(block.includes("min-h-[44px]"), "composer input should keep comfortable breathing room")
  assert.ok(block.includes("inset-y-0 left-1.5") && block.includes("inset-y-0 right-1.5"), "attach and send controls should stay vertically centered inside the input")
  ...
  assert.ok(block.includes("absolute inset-y-0 left-1.5"), "attachment control should stay vertically centered inside the input field")
  assert.ok(block.includes("absolute inset-y-0 right-1.5"), "send or stop control should stay vertically centered inside the input field")
