...
test("new chat surfaces preserve Aurora conversation ergonomics", () => {
  ...
  assert.ok(block.includes("sm:pb-[12px]"), "desktop message items should reserve paint-safe space for the lowered zero-height hover actions")
  assert.ok(block.includes("sm:top-[3px]"), "desktop timestamps and actions should sit slightly below the message content")
  ...
  assert.ok(block.includes("block max-h-[112px] min-h-[44px]"), "composer textarea should be block-level with comfortable breathing room and no baseline gap")
  assert.ok(block.includes("left-1.5 top-1/2") && block.includes("right-1.5 top-1/2") && block.includes("-translate-y-1/2"), "attach and send controls should use deterministic cross-browser vertical centering")
  assert.ok(block.includes("filled={hasComposerPayload}"), "send should become filled only when there is payload to send")
  assert.ok(block.includes("aurora-chat-composer__toolbar"), "model and reasoning controls should live in a quiet integrated toolbar")
  assert.ok(block.includes("aurora-chat-hints hidden") && block.includes("sm:flex"), "command hints should collapse away on mobile")
  ...
  assert.ok(block.includes("absolute left-1.5 top-1/2"), "attachment control should stay vertically centered inside the input field")
  assert.ok(block.includes("absolute right-1.5 top-1/2"), "send or stop control should stay vertically centered inside the input field")
